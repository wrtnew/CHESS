...
class ImageFolder_mtt(datasets.DatasetFolder):
    def __init__(self,
        ...
        super(ImageFolder_mtt, self).__init__(root,
                                          loader,
                                          self.extensions,
                                          transform=transform,
...
class ClassDataLoader(MultiEpochsDataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nclass =7#崔老师是7，ntu，uci是6，pafa是13，widar22
    ...
    def class_sample(self, c, ipc=-1):
        if ipc > 0:
            # 使用随机排列索引
            all_indices = self.cls_idx[c]
            # 真正随机选择 ipc 个索引
            sel_idx_indices = torch.randperm(len(all_indices))[:ipc]
            indices = [all_indices[i] for i in sel_idx_indices]
        else:
...
class ClassMemDataLoader():
    ...
    def __init__(self, dataset, batch_size, drop_last=False, device='cuda'):
        ...
        print([i for i in range(len(dataset))])
