...
from common import define_model, define_2k
...
def resnet10_bn(args, nclass, logger=None):
    model = RN.ResNet(args.dataset, 3, 7, 'batch', args.size, nch=args.nch)
    if logger is not None:
...
def densenet(args, nclass, logger=None):
    model = DN.densenet_cifar(nclass=7)
    return model

def mlp(args, nclass, logger=None):
    model = mlpp.MLP(num_classes=7)
    return model
...
def test_data_shu(args,
    ...
    """Train neural networks on condensed data
    """
    print('test shufflenet')
    if model_fn is None:
        model_fn_ls = [define_shu]
    else:
        model_fn_ls = [model_fn]
...
def test_data_in(args,
    ...
    print('test innet')
    if model_fn is None:
        model_fn_ls = [define_in]
        if test_resnet:
            model_fn_ls = [densenet]
    else:
        model_fn_ls = [model_fn]
...
def test_data_inception(args,
        ...
        """Train neural networks on condensed data
        """
        print('test inceptionnet')
        if model_fn is None:
            model_fn_ls = [define_incep]
            if test_resnet:
                model_fn_ls = [densenet]
        else:
            model_fn_ls = [model_fn]
...
def test_data(args,
              ...
              repeat=1,
              logger=print):
    """Train neural networks on condensed data and report mean ± std
    """
    if model_fn is None:
    ...
    for model_fn in model_fn_ls:
        ...
        best_mean, best_std = np.mean(best_acc_l), np.std(best_acc_l)
        # last_mean, last_std = np.mean(acc_l), np.std(acc_l)

        logger(
            f'Repeat {repeat} => '
            f'Best acc: {best_mean:.2f} ± {best_std:.2f}, '
            # f'Last acc: {last_mean:.2f} ± {last_std:.2f}\n'
        )

        results["best_acc"] = (best_mean, best_std)
        # results["last_acc"] = (last_mean, last_std)

    return best_mean

def test_data_5h(args,
              train_loader,
              val_loader,
              test_resnet=False,
              model_fn=None,
